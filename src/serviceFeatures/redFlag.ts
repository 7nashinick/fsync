...
export async function processVaultInitialisation(
    ...
    let completed = false;
    // Fsync fix: suspendAllSync() below zeroes every sync-mode flag and the
    // engine never restores them, which silently downgrades a live vault to
    // "On events" after a fetch/rebuild. Snapshot the intended mode now (the
    // onboarding has already persisted liveSync:true by this point) and
    // restore it once initialisation succeeds.
    const s0 = host.services.setting.currentSettings();
    const syncModeSnapshot = {
        liveSync: s0.liveSync,
        periodicReplication: s0.periodicReplication,
        syncOnSave: s0.syncOnSave,
        syncOnEditorSave: s0.syncOnEditorSave,
        syncOnStart: s0.syncOnStart,
        syncOnFileOpen: s0.syncOnFileOpen,
        syncAfterMerge: s0.syncAfterMerge,
        batchSave: s0.batchSave,
    };
    try {
        ...
        // Fsync fix: restore the pre-suspension sync mode on success, so a
        // live vault stays live after fetch/rebuild with no manual toggle.
        if (completed) {
            await host.services.setting.applyPartial(syncModeSnapshot, true);
        }
    }
}
